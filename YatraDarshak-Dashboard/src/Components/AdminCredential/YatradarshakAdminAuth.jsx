...
const LeftPanel = () => (
  <div className="w-full lg:w-1/2 relative min-h-[420px] lg:min-h-[720px]">
    ...
    <div className="absolute top-16 left-8 right-8 text-left lg:pl-12">
      <h2 className="text-white text-2xl lg:text-3xl font-bold leading-tight drop-shadow-lg">
        PARTNER WITH YATRA DARSHAK
      </h2>
      <p className="text-white/80 mt-3 max-w-md">
        Join our partner network — connect travelers with great local
        experiences, manage listings, and grow your business with tools designed
        for partners.
      </p>
    </div>
...
const YatradarshakAdminAuth = () => {
  ...
  useEffect(() => {
    ...
    if (token) navigate("/yd-partner");
  }, []);

  // ✅ Redirect after success
  useEffect(() => {
    if (showSuccess) {
      const timer = setTimeout(() => navigate("/yd-partner"), 1500);
      return () => clearTimeout(timer);
    }
  }, [showSuccess]);
  ...
  const handleSendOtp = async () => {
    ...
    try {
      setLoading(true);
      await sendAdminOtp({ phonenumber: formData.phoneNumber });
      setShowOtp(true);
      setOtpTimer(30);
      setError("");
    } catch (err) {
...
export default YatradarshakAdminAuth;
